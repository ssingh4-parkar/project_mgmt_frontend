...
function Sidebar({ isOpen }: { isOpen: boolean }) {
    const location = useLocation();
    const { permissions, isLoading, logout, user } = useAuth(); // Destructure 'user' as well
            ...
            {user && (
                <div className="p-4 border-t border-gray-700">
                    <div className="flex items-center mb-4">
                        {/* Use photoUrl if available, otherwise a placeholder */}
                        <img 
                            src={user.photoUrl || `ui-avatars.com{user.name}&background=random&color=fff`} 
                            alt={user.name} 
